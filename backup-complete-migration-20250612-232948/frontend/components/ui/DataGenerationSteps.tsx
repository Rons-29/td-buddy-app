...
export function DataGenerationSteps({ steps, currentStepId }: DataGenerationStepsProps) {
  ...
  const getStepColor = (status: DataGenerationStep['status']) => {
    switch (status) {
      ...
      case 'active':
        return 'text-brew-primary-600 bg-brew-primary-50 border-brew-primary-200';
      case 'error':
  ...
  return (
    <div className="bg-white rounded-xl border border-brew-primary-100 p-4 shadow-sm">
      <h3 className="text-sm font-semibold text-brew-primary-800 mb-3 flex items-center gap-2">
        🍺 <span>TDの作業進捗</span>
      </h3>
      ...
      {/* 全体的な進捗表示 */}
      <div className="mt-3 pt-3 border-t border-brew-primary-100">
        <div className="flex items-center justify-between text-xs text-brew-primary-600">
          <span>全体進捗</span>
          <span>
