...
export function QRCodePreview({ text, className = '' }: QRCodePreviewProps) {
  ...
  return (
    <div className={`bg-white rounded-lg border border-brew-primary-200 p-6 ${className}`}>
      <div className="text-center">
        <h4 className="text-lg font-semibold text-brew-primary-800 mb-4 flex items-center justify-center">
          📱 QRコード
        </h4>
        ...
        <div className="mt-4 p-3 bg-td-gray-50 rounded border border-td-gray-200 text-sm">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <span className="ml-1 font-mono text-brew-primary-700">{truncationInfo.originalLength}</span>
            </div>
            <div>
              <span className="text-td-gray-600">QR用長さ:</span>
              <span className="ml-1 font-mono text-brew-primary-700">{truncationInfo.safeLength}</span>
            </div>
            <div>
              <span className="text-td-gray-600">推奨サイズ:</span>
              <span className="ml-1 font-mono text-brew-primary-700">{getRecommendedQRSize(text)}px</span>
            </div>
            <div>
              <span className="text-td-gray-600">バイト数:</span>
              <span className="ml-1 font-mono text-brew-primary-700">{new TextEncoder().encode(truncationInfo.truncatedText).length}</span>
            </div>
          </div>
        </div>
