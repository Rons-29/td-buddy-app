...
echo "🤖 TDからのメッセージ: 生成→生成の一括変換を開始します！"
echo ""
...
declare -a PATTERNS=(
  ...
  "s/CSV.*生成/CSV生成/g"
  "s/UUID.*生成/UUID生成/g"
  "s/AI連携型テストデータ生成ツール/AI連携型テストデータ生成ツール/g"
  "s/テスト用.*生成/テストデータ生成/g"
)
...
if [ -f "package.json" ]; then
  sed -i.tmp 's/データ生成から品質管理まで/データ生成から品質管理まで/g' package.json
...
if [ -f "package-root.json" ]; then
  sed -i.tmp 's/データ生成から品質管理まで/データ生成から品質管理まで/g' package-root.json
...
for script_file in scripts/*.sh; do
  if [ -f "$script_file" ] && [ "$script_file" != "scripts/convert-brewing-to-generation.sh" ]; then
    sed -i.tmp 's/生成 → 生成/生成 → 生成/g' "$script_file"
    sed -i.tmp 's/生成→生成/生成→生成/g' "$script_file"
...
echo "  git status"
echo ""
echo "🤖 TDからのメッセージ: 生成→生成変換が完了しました！"
