...
router.post('/generate', async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (validationError) {
      const response: ApiResponse = {
        ...
        error: {
          ...
          statusCode: 400,
          tdMessage: '🍺 Brew: 入力内容を確認してください。正しい形式で入力し直してくださいね',
    ...
    const response: ApiResponse = {
      ...
      tdMessage: `🍺 Brew: ${criteria.count}個の${criteria.version}バージョンUUIDを${criteria.format}形式で生成しました！品質も完璧です♪`,
      timestamp: new Date().toISOString(),
      metadata: {
    ...
    const response: ApiResponse = {
      ...
      error: {
        ...
        statusCode: 500,
        tdMessage: '🍺 Brew: UUID生成でエラーが発生しました。しばらく時間をおいて再試行してください',
        timestamp: new Date().toISOString(),
        path: req.path,
        method: req.method
      },
...
router.post('/validate', async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (validationError) {
      const response: ApiResponse = {
        ...
        error: {
          ...
          message: validationError,
          statusCode: 400,
          tdMessage: '🍺 Brew: UUID検証のリクエスト形式を確認してください',
          timestamp: new Date().toISOString(),
          path: req.path,
          method: req.method
        },
    ...
    const response: ApiResponse = {
      ...
      tdMessage: `🍺 Brew: ${result.summary.totalChecked}個のUUIDを検証しました。有効: ${result.summary.validCount}個、無効: ${result.summary.invalidCount}個です`,
      timestamp: new Date().toISOString(),
      metadata: {
        duration: Date.now() - startTime
      }
    ...
    const response: ApiResponse = {
      ...
      error: {
        ...
        statusCode: 500,
        tdMessage: '🍺 Brew: UUID検証でエラーが発生しました。入力形式を確認してください',
        timestamp: new Date().toISOString(),
        path: req.path,
        method: req.method
      },
...
router.get('/history', async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (limit > 200) {
      const response: ApiResponse = {
        ...
        error: {
          ...
          statusCode: 400,
          tdMessage: '🍺 Brew: 一度に取得できる履歴は200件までです',
          timestamp: new Date().toISOString(),
          path: req.path,
          method: req.method
        },
    ...
    const response: ApiResponse = {
      ...
      tdMessage: `🍺 Brew: ${result.history.length}件の履歴を取得しました（全${result.total}件中）`,
      timestamp: new Date().toISOString(),
      metadata: {
    ...
    const response: ApiResponse = {
      ...
      error: {
        ...
        statusCode: 500,
        tdMessage: '🍺 Brew: 履歴の取得でエラーが発生しました',
        timestamp: new Date().toISOString(),
        path: req.path,
        method: req.method
      },
...
router.get('/statistics', async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    const response: ApiResponse = {
      ...
      tdMessage: `🍺 Brew: 累計${result.totalGenerated}個のUUIDが生成されています！`,
      timestamp: new Date().toISOString(),
      metadata: {
        duration: Date.now() - startTime
      }
    ...
    const response: ApiResponse = {
      ...
      error: {
        ...
        statusCode: 500,
        tdMessage: '🍺 Brew: 統計情報の取得でエラーが発生しました',
        timestamp: new Date().toISOString(),
        path: req.path,
        method: req.method
      },
