...
import { PersonalInfoItem } from '../types/personalInfo';
...
describe('🎯 Data Quality Tests', () => {
  ...
  describe('🔤 氏名品質検証', () => {
    test('✅ 漢字氏名の形式検証', async () => {
      ...
      result.data.forEach(person => {
        ...
        // 姓名の組み合わせが正しい
        expect(person.fullName.kanji).toBe(`${person.fullName.lastName} ${person.fullName.firstName}`);
      });
    });
    ...
    test('✅ 氏名の多様性検証', async () => {
      ...
      const lastNameDuplicationRate = (lastNames.length - uniqueLastNames.size) / lastNames.length;
      const firstNameDuplicationRate = (firstNames.length - uniqueFirstNames.size) / firstNames.length;
  ...
  describe('📧 メールアドレス品質検証', () => {
    ...
    test('✅ テストドメインの使用確認', async () => {
      ...
      const testDomains = [
        'example.com', 'test.com', 'sample.net', 'prototype.test',
        'qa-test.invalid', 'beta-test.dev', 'temp-mail.demo',
        'virtual-user.test', 'unit-test.example', 'system-test.mock',
        'integration.test', 'test-mail.jp'
      ];
...
// TD からのメッセージ
console.log('🤖 TD: データ品質テストを実行中です！生成データの品質を徹底的にチェックしましょう♪'); 