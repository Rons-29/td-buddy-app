...
export const TDDesignInspector: React.FC = () => {
  ...
  useEffect(() => {
    const handleClick = (event: MouseEvent) => {
      ...
      if (target.closest('.td-inspector')) return; // インスペクター自体は除外
    ...
    const handleMouseOver = (event: MouseEvent) => {
      ...
      const target = event.target as HTMLElement;
      if (target.closest('.td-inspector')) return;
...
- Styles: ${JSON.stringify(elementInfo.styles, null, 2)}
  ...
  return (
    <>
      ...
      <div className={`td-inspector fixed bottom-4 right-4 z-50 ${isInspectMode ? 'td-inspect-cursor' : ''}`}>
        {/* メインボタン */}
        <button
        ...
        {isOpen && (
          <div className="absolute bottom-16 right-0 w-96 bg-white rounded-lg shadow-2xl border-2 border-td-primary-200 p-4 max-h-96 overflow-y-auto">
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-td-primary-800">🎨 TD Design Inspector</h3>
              <div className="flex gap-2">
                <button
                  onClick={() => setShowGrid(!showGrid)}
                  className={`p-2 rounded transition-colors ${showGrid ? 'bg-td-primary-600 text-white' : 'bg-gray-200 text-gray-600'}`}
                  title="グリッド表示"
                >
                ...
                <button
                  ...
                  className={`p-2 rounded transition-colors ${isInspectMode ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-600'}`}
                  title="検査モード"
                >
                  {isInspectMode ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                </button>
              </div>
            </div>
            ...
            {elementInfo && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-semibold text-gray-800">選択された要素</h4>
                  <button
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div>
                    ...
                    <span className={`ml-1 px-2 py-1 rounded text-xs ${elementInfo.tdComponent ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      {elementInfo.tdComponent || 'なし'}
                    </span>
                  </div>
                  <div>
                    <strong>Test ID:</strong>
                    <span className={`ml-1 px-2 py-1 rounded text-xs ${elementInfo.testId ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-2 rounded text-xs space-y-1">
                    <div>サイズ: {elementInfo.styles.width} × {elementInfo.styles.height}</div>
            ...
            <div className="mt-4 p-3 bg-td-secondary-50 rounded-lg">
              <p className="text-sm text-td-secondary-700">
                💡 <strong>TDのヒント:</strong> TD Design Systemの要素には <code>data-td-component</code> 属性を追加すると、より詳細な情報が表示されます！
              </p>
            </div>
          </div>
        )}
