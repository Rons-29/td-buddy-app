...
export type TDMood = 'happy' | 'excited' | 'working' | 'thinking' | 'success' | 'error' | 'sleeping';
...
const EnhancedTDCharacter: React.FC<EnhancedTDCharacterProps> = ({
  ...
  const [currentMood, setCurrentMood] = useState<TDMood>(mood);
  ...
  const moodEmojis: Record<TDMood, string> = {
    happy: '😊',
    excited: '🤩',
    working: '🔧',
    thinking: '🤔',
    ...
    <div className={cn('relative flex flex-col items-center', className)}>
      ...
      {showSpeechBubble && currentMessage && (
        <div className="relative mb-4 max-w-xs">
          <div className="bg-white/95 backdrop-blur-sm border-2 border-td-primary-200 rounded-2xl p-4 shadow-lg">
            <p className="text-sm text-td-primary-800 font-medium text-center">
              {currentMessage}
            </p>
          </div>
          {/* Speech bubble tail */}
      ...
      <div
        ...
        className={cn(
          ...
          'bg-gradient-to-br from-td-primary-100 to-td-primary-200',
          'border-4 border-td-primary-300 shadow-xl',
      ...
      >
        ...
        {/* Glow effect */}
        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-td-primary-400/20 to-td-accent-400/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <div className={cn(
        ...
        'bg-td-primary-100 text-td-primary-800 border border-td-primary-200'
      )}>
