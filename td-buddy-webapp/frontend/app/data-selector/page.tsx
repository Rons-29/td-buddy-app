...
export default function DataSelectorPage() {
  ...
  const handleSelectionChange = useCallback((items: SelectableDataItem[]) => {
    ...
    if (items.length === 0) {
      setTdMessage('データを選択してください。チェックボックスをクリックするか、全選択ボタンをお試しください♪');
    } else if (items.length === 1) {
      setTdMessage(`1件のデータを選択中です。種類: ${items[0].type}, カテゴリ: ${items[0].category}`);
    } else {
      setTdMessage(`${items.length}件のデータを選択中です。一括コピーやダウンロードが可能です♪`);
    }
  }, []);
  ...
  const handleBulkAction = useCallback((action: 'copy' | 'download' | 'delete', items: SelectableDataItem[]) => {
    switch (action) {
      case 'copy':
        ...
        setTdMessage(`✅ ${items.length}件のデータをクリップボードにコピーしました！`);
        break;
      ...
      case 'download':
        ...
        setTdMessage(`📥 ${items.length}件のデータをCSVファイルでダウンロードしました！`);
        break;
      ...
      case 'delete':
        ...
        setSelectedItems([]);
        setTdMessage(`🗑️ ${items.length}件のデータを削除しました。`);
        break;
    }
  }, [sampleData]);
  ...
  const regenerateData = useCallback(() => {
    ...
    setSelectedItems([]);
    setTdMessage('🔄 サンプルデータを再生成しました！新しいデータをお試しください♪');
  }, []);
